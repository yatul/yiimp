        ...
        foreach ($balances as $balance) {
            ...
            if ($updatebalances) {
                ...
                foreach ($coins as $coin) {
                    ...
                    if ($coin->installed && empty($market->deposit_address)) {
                        ...
                        if($address) {
                            $market->deposit_address = str_replace('"',"", $address);
                        }
                    }
...
function doGraviexTrading($quick=false)
{
    ...
    $sell_ask_pct = 0.9999;     // sell on ask price + 5%
    ...
    if (is_array($balances)) {
        ...
        foreach ($balances as $balance) {
            ...
            if (!$res || (is_array($res) && !empty($res["error"])) || !empty($res->error)) {
